...
static int quick_sort_util(int* array, int l, int r){
...
static void topk_util(int k, int* array, int len) {

	int p = quick_sort_util(array, 0, len);

	while (p != k) {
		if (p > k) {
			p = quick_sort_util(array, 0, p);
		}
		else { //p < k
			k = k - p;
			p = quick_sort_util(array, p, len);
		}
	}
